...
describe("isEven test", () => {
    ...
    test.each(oddNumbers)(
        "isEven function fails for non-even value %j",
        (num) => expect(exercise.isEven(num)).toBeFalsy()
    );
...
//ex2
describe("removeAtLeastOne test", () => {
    ...
    test.each(removeAtLeastOneEdgeCases)(
        ...
        (input) =>
            expect(exercise.removeAtLeastOne(input.arr).error).toBe(
                "invalid input"
            )
    );
});
...
describe("validate test", () => {
    ...
    test.each(validateEdgeCases)(
        "validate function fails for value %j",
        (input) =>
            expect(exercise.validate(input).error).toBe(
                "Need at least one boolean"
            )
    );
});
